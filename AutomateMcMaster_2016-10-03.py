import pyperclip, re, openpyxl, os
...
itemReferenceRegex = re.compile(r'Your Reference: ?\S\n')
...
#     print('Too many items to fit in one spreadsheet. Consider splitting into two or more spreadsheets.')
    ...
    for x in range(0, numberOfItems):
        ...
        itemReferenceSearch = itemReferenceRegex.search(text, 0, 50)
        itemReferencesStr.append(itemReferenceSearch.group())
    ...
    spreadsheetsRequired = ((numberOfItems-1)//16)+1
    counterMin = 0

    if spreadsheetsRequired > 1:
        counterMax = 16
    else:
        counterMax = numberOfItems

    for spreadsheetNumber in range(0, spreadsheetsRequired):
        wb = openpyxl.load_workbook('purchaseorder.xlsx')
        ...
        for x in range(counterMin, counterMax):
            sheet['A' + str(x+20-counterMin)] = itemQuantitiesStr[x]
            sheet['B' + str(x+20-counterMin)] = itemNumbersStr[x]
            sheet['C' + str(x+20-counterMin)] = itemDescriptionsStr[x]
            sheet['G' + str(x+20-counterMin)] = itemPricesStr[x]
            if len(itemDescriptionsStr[x]) > 30:
                heightFactor = len(itemDescriptionsStr[x]) // 30
                sheet.row_dimensions[x+20-counterMin].height = 12 * heightFactor
            else:
                sheet.row_dimensions[x+20-counterMin].height = 12

        if spreadsheetsRequired is 1:
            wb.save('Finished_PO.xlsx')
            print('Exporting to \'Finished_PO.xlsx\'')
        else:
            wb.save('Finished_PO_Part' + str(spreadsheetNumber+1) + '.xlsx')
            print('Exporting to \'Finished_PO_Part' + str(spreadsheetNumber+1) + '.xlsx\'')
            counterMin = counterMax
            if spreadsheetNumber is spreadsheetsRequired-2:
                counterMax += numberOfItems % 16
                print('gotem')
            else:
                counterMax = counterMin + 16
